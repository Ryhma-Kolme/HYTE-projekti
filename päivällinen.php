...
<main>
    <div class="row-buttons">
        ...
        <button class="current" onclick="document.location = 'päivällinen.php'">Päivällinen</button>
        <button onclick="document.location = 'iltapala.php'">Iltapala</button>
    </div>

    <div class="header-white-ruoka">
        <div class="row-1">
            <p>Lisää päivällisesi valitsemalla tallennettu ruoka-aine tai lisää uusi.</p>
    ...
    <div class="row">
        <div class="column">
            ...
            <div class="column-content">
                ...
                <?php // Poistaa kaikki tämän päivän päivälliset "Poista kaikki"-napista
                    if(isset($_POST['din_deletebtn'])){

                        // Näytetään vain kirjautuneen käyttäjän ja tämän päivän lisätyt ruoka-aineet
                        $sql="DELETE FROM app_dinner WHERE DATE(`timeOfEating`) = '$clickedDay' AND userID = '$currentUserID' ORDER BY timeOfEating ASC";
                    ...
                    if(isset($_POST['foodbtn'])){
                        ...
                        if ($selected_val==NULL) {
                            ...
                            echo("<h2>Et ole lisännyt ruoan määrää. Lisää määrä ennen tallentamista. </h2>");               
                                ...
                                $STH = $DBH->prepare("INSERT INTO app_dinner 
                ...
                <?php // Syötetään arvot taulukkoon
                    ...
                    // Näytetään vain kirjautuneen käyttäjän ja tämän päivän lisätyt ruoka-aineet
                    $sql="SELECT * FROM app_dinner WHERE DATE(`timeOfEating`) = '$clickedDay' AND userID = '$currentUserID' ORDER BY timeOfEating ASC";
                    $kysely=$DBH->prepare($sql);				
                    $kysely->execute();
